...
Object.assign(Compiler.prototype, {
    ...
    _createWatcher (opts) {
        ...
        watcher.on('all', (event, path) => {
            ...
            let find = this._scriptsCache.find(s => {
                return s.src === path;
            })
            if (!find) return;
    ...
    getDstPath (path) {
        ...
        return Path.join(this.out, Path.stripExt(relative) + '.js').replace(/\\/g, '/');
    },
    ...
    getNodeModuleDstPath (path) {
        let relative = Path.join(TEMP_NODE_MODULE_PATH, path.substring(path.indexOf('/node_modules/')+'/node_modules/'.length, path.length));
    ...
    compileScripts (paths, cb) {
        ...
        paths.forEach(path => {
            if (this._scriptsToCompile.indexOf(path) === -1) {
                this._scriptsToCompile.push(path);
            }
        });
    ...
    removeScripts (paths, uuids) {
        ...
        this._scriptsToCompile = this._scriptsToCompile.filter(path => paths.indexOf(path) === -1);
        this._scriptsCache = this._scriptsCache.filter(script => tempPaths.indexOf(script.src) === -1);
